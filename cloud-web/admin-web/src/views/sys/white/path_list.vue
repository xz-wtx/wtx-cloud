<template>
  <x_init>
    ...
    >
      <path_edit :table-data="tableData"></path_edit>
    </el-dialog>
  </x_init>
</template>
...
import Path_edit from "@/views/sys/white/path_edit.vue";
import userCommon from "@/plugins/useCommon";


export default {
  name: "path_list",
  components: {Path_edit},
  setup(){
    ...
    xTableData.search.fields={
      path: {placeholder: '路径', type: "input", title: '路径'},
    ...
    let methods={
      //加载数据
      loadData(obj){
        proxy.$api.admin.path_api.getPageList(obj).then(res=>{
    ...
    function upStatus(obj,but) {
      upState(obj,but,tableData,proxy.$api.admin.path_api.del);
    }
